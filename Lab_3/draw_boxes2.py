...
def draw_boxes2(image, boxes, box_classes, class_names, scores=None):
    ...
    # Nếu image là ảnh PIL, chuyển thành mảng NumPy trước
    if isinstance(image, Image.Image):
        image = np.array(image)
    elif not isinstance(image, np.ndarray):
        raise TypeError(f"Unsupported image type: {type(image)}")

    # Chuẩn hóa và chuyển lại thành ảnh PIL
    if image.dtype != np.uint8:
        image = np.floor(image * 255 + 0.5).astype('uint8')
    image = Image.fromarray(image)

    # ✅ FIXED: Load font with fallback
    try:
        ...
        print("⚠️ Used default font (original font not found)")
